    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/basic/area/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.area.name
                this.dataForm.bdOrgId = data.area.bdOrgId
                this.dataForm.createUserId = data.area.createUserId
                this.dataForm.createTime = data.area.createTime
                this.dataForm.remark = data.area.remark
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/basic/area/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',

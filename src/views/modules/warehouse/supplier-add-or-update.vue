  ...
  >
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="80px" @keyup.enter.native="dataFormSubmit()">
      ...
      <el-form-item label="联系人" prop="contactName">
        <el-input v-model="dataForm.contactName" placeholder="联系人" />
      </el-form-item>
      <el-form-item label="联系人电话" prop="contactMobile">
        <el-input v-model="dataForm.contactMobile" placeholder="联系人电话" />
      </el-form-item>
      <el-form-item label="供应商地址" prop="address">
        <el-input v-model="dataForm.address" placeholder="供应商地址" />
      </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/warehouse/supplier/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.supplier.name
                this.dataForm.contactName = data.supplier.contactName
                this.dataForm.contactMobile = data.supplier.contactMobile
                this.dataForm.address = data.supplier.address
                this.dataForm.bdOrgId = data.supplier.bdOrgId
                this.dataForm.createUserId = data.supplier.createUserId
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/warehouse/supplier/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
